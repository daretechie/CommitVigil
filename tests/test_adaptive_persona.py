...
async def test_adaptive_persona_drafting():
    ...
    """
    with (
        patch("src.agents.brain.get_cultural_persona", new_callable=AsyncMock) as mock_get,
        ...
        mock_response.choices = [AsyncMock(message=AsyncMock(content="Auto Drafted Instruction"))]
...
async def test_adapt_tone_uses_persona():
    ...
    """
    mock_persona = CulturalPersona(
        code="ja", name="Japanese", instruction="Strict Japanese Instruction", is_verified=True
    )

    with (
        patch.object(
            CommitVigilBrain, "get_or_create_persona", new_callable=AsyncMock
        ) as mock_get_persona,
        ...
        await brain.adapt_tone(
            excuse=ExcuseAnalysis(
                category=ExcuseCategory.LEGITIMATE, confidence_score=0.9, reasoning="Reasons"
            ),
            risk=RiskAssessment(
                risk_score=0.1,
                level=RiskLevel.LOW,
            ...
            burnout=BurnoutDetection(
                is_at_risk=False, sentiment_indicators=[], recommendation="None"
            ),
            lang="ja",
        )
