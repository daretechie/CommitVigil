...
async def test_adversarial_hard_block_salary(mock_brain_components):
    ...
    with patch("src.agents.brain.SafetySupervisor") as MockSupervisor:
        ...
        mock_instance.audit_message = AsyncMock(return_value=SafetyAudit(
            is_safe=False,
...
async def test_nested_hr_context(mock_brain_components):
    ...
    nested_hr_msg.message = "The client asked about our team's salary expectations for the project bid."
...
async def test_no_infinite_corrections(mock_brain_components):
    ...
    with patch("src.agents.brain.SafetySupervisor") as MockSupervisor:
        ...
        print(f"[Test Output] Supervisor Call Count: {mock_instance.audit_message.call_count} (Pass)")
