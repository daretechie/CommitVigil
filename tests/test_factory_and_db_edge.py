...
from src.schemas.agents import UserHistory, SafetyRule, ExcuseCategory, ToneType
...
async def test_database_cooling_off_reset():
    ...
    past_time = datetime.now(timezone.utc) - timedelta(hours=settings.COOLING_OFF_PERIOD_HOURS + 1)
...
async def test_database_hierarchical_safety_rules():
    ...
    with patch("src.core.database.AsyncSessionLocal") as mock_session_factory:
        ...
        mock_result.scalar_one_or_none.side_effect = [None, SafetyRule(industry="engineering", department="*", semantic_rules="test")]
        mock_session.execute.return_value = mock_result
        ...
        mock_result.scalar_one_or_none.side_effect = [None, None, SafetyRule(industry="generic", department="*", semantic_rules="generic test")]
        mock_session.execute.return_value = mock_result
...
async def test_database_redis_cache_logic():
    ...
    mock_redis.get.return_value = '{"industry": "cached", "department": "*", "semantic_rules": "rules"}'
    ...
    with patch("src.core.database.AsyncSessionLocal") as mock_session_factory:
        ...
        mock_session_factory.return_value.__aenter__.return_value = mock_session
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = SafetyRule(industry="uncached", department="*", semantic_rules="rules")
        mock_session.execute.return_value = mock_result
