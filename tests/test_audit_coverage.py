...
from src.schemas.agents import ToneType, ProspectProfile, CorrectionFeedback
...
async def test_safety_supervisor_dynamic_rules():
    ...
    with patch("src.agents.learning.SupervisorFeedbackLoop.calculate_intervention_acceptance", return_value=0.5):
        result = await supervisor.audit_message(
            "test message", ToneType.FIRM, "test context", 
            industry="generic", context_profile=mock_context
        )
...
async def test_reports_api_departmental_audit():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        response = await ac.get(
            "/api/v1/reports/department/engineering",
            headers={"X-API-Key": settings.API_KEY_SECRET}
        )
        assert response.status_code == 200
...
async def test_reports_api_predict_roi():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        response = await ac.get(
            "/api/v1/demo/predict-roi?team_size=50",
            headers={"X-API-Key": settings.API_KEY_SECRET}
        )
        assert response.status_code == 200
